...
###Appendix###
Gam model with just smoothened
```{r}

#"val_alcohol_use_disorders","val_drug_use_disorders","val_depressive_disorders"
data.gam = gam(val_depressive_disorders ~ s(val_alcohol_use_disorders) + s(val_drug_use_disorders), data = m_data, method = "REML")
data.gam.df = augment(data.gam)
print(summary(data.gam))
  
#drug_use_disorders
#ggplot(data.gam.df, aes(x=val_drug_use_disorders,y=.resid)) + geom_point() + geom_smooth(method = "loess") +
#  xlab("Drug Disorder population (in percent)") +
#  ggtitle('Residual plot with drug disorder population') 

#alcohol_use_disorders
#ggplot(data.gam.df, aes(x=val_alcohol_use_disorders,y=.resid)) + geom_point() + geom_smooth(method = "loess") +
#  xlab("Alcohol Disorder population (in percent)") +
#  ggtitle('Residual plot with alcohol disorder population') 

data.grid2 <- expand.grid(val_alcohol_use_disorders = 0.5:3.5,val_drug_use_disorders = 0.25:1.5)
data.pred = predict(data.gam,newdata = data.grid2)
data.grid2$depression_pred = as.vector(data.pred)
data.plot.df <- data.frame(data.grid2)

ggplot(data.plot.df,aes(x = val_alcohol_use_disorders, y = val_drug_use_disorders, z=depression_pred, fill=depression_pred))+geom_raster()+
  scale_fill_distiller(palette = "RdYlBu") +
  ylab("drug_use_disorder(in percent)")+
  xlab("alcohol_use_disorder(in percent)") +
  #geom_contour() +
#geom_text_contour() +
  theme_bw() +
  ggtitle('Figure 4.3: Raster Plot between substance disorder and depression percent')
...
#"val_alcohol_use_disorders","val_drug_use_disorders","val_depressive_disorders"
data.gam = gam(val_depressive_disorders ~ s(val_alcohol_use_disorders,val_drug_use_disorders), data = m_data, method = "REML")
data.gam.df = augment(data.gam)
summary(data.gam)
...
ggplot(data.plot.df,aes(x = val_alcohol_use_disorders, y = val_drug_use_disorders, z=depression_pred, fill=depression_pred))+ geom_raster() +
   scale_fill_distiller(palette = "RdYlBu") +
#   ylab("drug_use_disorder")+
#   xlab("alcohol_use_disorder") +
  geom_contour() +
# geom_text_contour() +
 theme_bw() 
